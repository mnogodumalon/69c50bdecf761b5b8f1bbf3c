...
const TESTFALLERFASSUNG_FIELDS = [
  { key: 'testfall_name', label: 'Testfallname', type: 'string/text' },
  { key: 'testfall_beschreibung', label: 'Beschreibung', type: 'string/textarea' },
  { key: 'software_version', label: 'Softwareversion', type: 'string/text' },
  { key: 'testdatum', label: 'Testdatum', type: 'date/date' },
  ...
  { key: 'teststatus', label: 'Teststatus', type: 'lookup/select', options: [{ key: 'offen', label: 'Offen' }, { key: 'in_bearbeitung', label: 'In Bearbeitung' }, { key: 'abgeschlossen', label: 'Abgeschlossen' }, { key: 'blockiert', label: 'Blockiert' }] },
  { key: 'testergebnis', label: 'Testergebnis', type: 'lookup/radio', options: [{ key: 'bestanden', label: 'Bestanden' }, { key: 'fehlgeschlagen', label: 'Fehlgeschlagen' }, { key: 'nicht_getestet', label: 'Nicht getestet' }] },
  { key: 'tester_vorname', label: 'Vorname des Testers', type: 'string/text' },
  { key: 'tester_nachname', label: 'Nachname des Testers', type: 'string/text' },
...
const ENTITY_TABS = [
  { key: 'testfall_erfassung', label: 'Testfall-Erfassung', pascal: 'TestfallErfassung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('testfall_erfassung');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'testfall_erfassung': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testfall_erfassung': return (data as any).testfallErfassung as TestfallErfassung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testfall_erfassung': return TESTFALLERFASSUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'testfall_erfassung': return {
        create: (fields: any) => LivingAppsService.createTestfallErfassungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTestfallErfassungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTestfallErfassungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'testfall_erfassung' || dialogState?.entity === 'testfall_erfassung') && (
        <TestfallErfassungDialog
          open={createEntity === 'testfall_erfassung' || dialogState?.entity === 'testfall_erfassung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'testfall_erfassung' ? handleUpdate : (fields: any) => handleCreate('testfall_erfassung', fields)}
          defaultValues={dialogState?.entity === 'testfall_erfassung' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['TestfallErfassung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['TestfallErfassung']}
        />
      )}
      {viewState?.entity === 'testfall_erfassung' && (
        <TestfallErfassungViewDialog
          open={viewState?.entity === 'testfall_erfassung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'testfall_erfassung', record: r }); }}
        />
      )}
