...
import type { TestfallErfassung, CreateTestfallErfassung } from '@/types/app';
...
export class LivingAppsService {
  // --- TESTFALL_ERFASSUNG ---
  static async getTestfallErfassung(): Promise<TestfallErfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TESTFALL_ERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as TestfallErfassung[];
    return enrichLookupFields(records, 'testfall_erfassung');
  }
  static async getTestfallErfassungEntry(id: string): Promise<TestfallErfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TESTFALL_ERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as TestfallErfassung;
    return enrichLookupFields([record], 'testfall_erfassung')[0];
  }
  static async createTestfallErfassungEntry(fields: CreateTestfallErfassung) {
    return callApi('POST', `/apps/${APP_IDS.TESTFALL_ERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'testfall_erfassung') });
  }
  static async updateTestfallErfassungEntry(id: string, fields: Partial<CreateTestfallErfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.TESTFALL_ERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'testfall_erfassung') });
  }
  static async deleteTestfallErfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TESTFALL_ERFASSUNG}/records/${id}`);
  }
